...
def test_multi_loops_3d(x):
    ...
    sim_3d = dysim.Simulator (100, 20000, 600, np.pi)
...
def test_multi_loops_2d(x):
    ...
    sim_2d = dysim.Simulator (100, 20000, 600, np.pi)
...
if __name__ == '__main__':
    ...
    ax[0].set_title("$R_{pl}$ = 0.1 $R_{st}$, Orbit: 200, u: 0.6, e=0.5")
    ax[1].set_title('Residual')
    ax[1].legend()
    plt.suptitle('2D vs 3D transiting objects')
    np.savetxt('2d3d_0.1R_limb_kep.csv', np.transpose(np.array([frm, lc2d, lc3d])),delimiter=' ', header='frame, 2d, 3d')
    plt.savefig('2d3d_res_0.1R_limb_kep.png')
    #plt.show()
