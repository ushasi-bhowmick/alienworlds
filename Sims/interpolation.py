...
def formula(val, parspace, rorb, rpl, b, u1, u2):
    ...
    val2=[interp1d(parspace[4][:2], [val[i],val[i+1]],kind='linear')(u2) for i in range(0,len(val),2)]
    val2=[interp1d(parspace[3], [val2[i],val2[i+1],val2[i+2]], kind='quadratic')(u1) for i in range(0,len(val2)-2,3)]
    val2=[interp1d(parspace[2][:2], [val2[i],val2[i+1]], kind='linear')(b) for i in range(0,len(val2),2)]
    val2=[interp1d(parspace[1], [val2[i],val2[i+1],val2[i+2]], kind='quadratic')(rpl) for i in range(0,len(val2)-2,3)]
...
def lc_read(rpl, rorb, b, u1, u2):
    ...
    vs=[[rpl_arr[i],rpl_arr[i+1],rpl_arr[i+2]] for i in range(0,len(rpl_arr)-2) if(rpl<rpl_arr[i+2] and rpl>rpl_arr[i])][0]
    ...
    vs=[[b_arr[i],b_arr[i+1],0] for i in range(len(b_arr)-1) if(b<b_arr[i+1] and b>b_arr[i])][0]
    ...
    vs=[[u1_arr[i],u1_arr[i+1],u1_arr[i+2]] for i in range(len(u1_arr)-2) if(u1<u1_arr[i+2] and u1>u1_arr[i])][0]
    ...
    vs=[[u2_arr[i],u2_arr[i+1],0] for i in range(len(u2_arr)-1) if(u2<u2_arr[i+1] and u2>u2_arr[i])][0]
...
def lc_interpolate(rpl, rorb, b, u1, u2):
    ...
    return(ph_final, op_flux, phase_list, np.array(flux_list))
