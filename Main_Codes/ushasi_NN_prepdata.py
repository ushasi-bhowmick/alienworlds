...
def take_out_from_dv():
    ...
    for el in entry_fps:
        hdu=fits.open(FILEPATH_RAW_FPS+el.name)
        num=len(hdu)
        ...
        x=hdu[1].data["TIME"][int(datl*4/9):int(datl*5/9)]
        y=hdu[1].data["LC_WHITE"][int(datl*4/9):int(datl*5/9)]
        yr=hdu[1].data["LC_WHITE"][int(datl*4/9):int(datl*5/9)]
...
def get_labels_and_make_TS():
    ...
    np.savetxt('training_data/Xtrain_ush_short.csv', np.transpose(np.array(Xtrain)), delimiter=',')
    np.savetxt('training_data/Ytrain_ush_short.csv', np.array(Ytrain), delimiter=',')
